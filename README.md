...
不想開電腦時的另一條路：`docs/` 底下是同一套處理流程改寫成的純靜態網頁版，
上傳 MHT 快照、跑完八階段、產出成品 HTML，全部在手機瀏覽器裡完成，不需要電腦跑
`web.server`。部署步驟見 [DEPLOY.md](DEPLOY.md)（需要使用者自行決定是否公開倉庫）。
與上面「用手機開」的差別：那個方式仍要電腦開著伺服器、手機連同一 Wi-Fi；
這個版本部署一次後，之後隨時用手機開網址即可，兩者並存、互不影響。
